...
/**
 * Cloud-native servlet with externalized secrets using Azure Key Vault.
 * Fixes blocker: cr-java-0113, cr-java-0116
 */
...
public class WeatherServlet extends HttpServlet {
  ...
  // Weather API key externalized to environment variable (Azure Key Vault)
  // Fixes blocker: cr-java-0113
  ...
  protected void doGet(HttpServletRequest request,
    ...
    // Use externalized weather API key from Azure Key Vault via environment variable
    String apiKey = weatherApiKey != null && !weatherApiKey.isEmpty() 
        ? weatherApiKey 
        : System.getenv("WEATHER_API_KEY");
    
    String mockedKey = mockKey(apiKey);
    ...
    if (apiKey != null && apiKey.trim().length() > 0) {
      ...
      getRealTimeWeatherData(city, apiKey, response);
    } else {
      logger.info(
